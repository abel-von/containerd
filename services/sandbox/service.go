...
func (s *service) Create(ctx context.Context, req *api.CreateSandboxRequest) (*api.CreateSandboxResponse, error) {
	...
	sb, err := sandbox.SandboxFromProto(req.Sandbox)
	if err != nil {
		return nil, errdefs.ToGRPC(err)
	}
	...
	proto, err := sandbox.SandboxToProto(newSb)
	if err != nil {
		return nil, errdefs.ToGRPC(err)
	}
...
func (s *service) RemoveContainer(ctx context.Context, req *api.RemoveContainerRequest) (*types.Empty, error) {
...
func (s *service) Delete(ctx context.Context, req *api.DeleteSandboxRequest) (*types.Empty, error) {
...
func (s *service) List(ctx context.Context, req *api.ListSandboxRequest) (*api.ListSandboxResponse, error) {
	...
	var ret []api.Sandbox
	for _, sb := range sbs {
		newSb, err := sandbox.SandboxToProto(&sb)
		if err != nil {
			return nil, errdefs.ToGRPC(err)
		}
