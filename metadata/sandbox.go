		...
		defer func() {
			if retErr != nil {
				if err := s.ctrl.Shutdown(ctx, id); err != nil {
					log.G(ctx).Warnf("failed to shutdown vm when rollback, {}", err)
				}
			}
		}()
...
func (s *sandboxer) compareContainers(oldContainers, newContainers []sandbox.Container) ([]sandbox.Container, []sandbox.Container, []string) {
	var (
		appendedContainers []sandbox.Container
		updatedContainers  []sandbox.Container
		removedContainers  []string
	)
	for _, old := range oldContainers {
		found := false
		for _, new := range newContainers {
			if old.ID == new.ID {
				found = true
				if s.containerChanged(&old, &new) {
					updatedContainers = append(updatedContainers, new)
				}
				break
			}
		}
		if !found {
			removedContainers = append(removedContainers, old.ID)
		}
	}
	for _, new := range newContainers {
		found := false
		...
		if !found {
			appendedContainers = append(appendedContainers, new)
		}
	}
	return appendedContainers, updatedContainers, removedContainers
}
func (s *sandboxer) containerChanged(old, new *sandbox.Container) bool {
	// we don't compare Io, Bundle and Rootfs because these fields can be considered immutable
	if len(old.Processes) != len(new.Processes) {
		return true
	}
	if old.Spec.TypeUrl != new.Spec.TypeUrl || bytes.Compare(old.Spec.Value, new.Spec.Value) != 0 {
		return true
	}
	return false
}
...
func (s *sandboxer) writeContainers(parent *bbolt.Bucket, containers []sandbox.Container) error {
	bucket, err := parent.CreateBucketIfNotExists([]byte("containers"))
	if err != nil {
		return err
	}
...
func (s *sandboxer) writeProcesses(parent *bbolt.Bucket, processes []sandbox.Process) error {
	bucket, err := parent.CreateBucketIfNotExists([]byte("processes"))
	if err != nil {
		return err
	}
